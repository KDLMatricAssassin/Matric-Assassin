...
function initializeGameData() {
    ...
    const playerData = `
        ...
        Seth Sklar --> Gideon Bloom\tAVZQTV
        Gideon Bloom --> Josh Galgut\tQTUVNO
        ...
        Rebbeca Jellin --> Levi Sweidan\t6MZSHY
        Levi Sweidan --> Sabrina Cohen\t0DE5TC
        ...
        Bailey Lipworth --> Leah Kerr-Phillps\tW92ATY
        Leah Kerr-Phillips --> Jayden Sacker\t2HE5XC
        ...
        Eden Dworcan --> Adam Gordon\tF7B2X1
        Adam Gordon --> Sienna Silberman\tF4TG9S
        Sienna Silberman --> Ben Meltzer\tPK3DR9
        Ben Meltzer --> Ari Symons\tJ8GF4D
        Ari Symons --> Tyler Cimring\tH5NM2X
        Tyler Cimring --> Jacob Defries\tX7L9QM
        Jacob Defries --> Hannah Benson\tD3TZV5
        Hannah Benson --> Leora Joffe\tL81RKU
    `;
...
function applySuspension(playerId, days) {
    ...
    // Close the modal
    document.getElementById('suspend-player-modal').classList.remove('active');
    ...
    showNotification(
        `Player ${player.name} has been suspended for ${durationText} (until ${new Date(suspendedUntil).toLocaleDateString()})`,
        'success'
    );
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Add test button for target elimination
    const testButton = document.createElement('button');
    testButton.textContent = 'Test Target Elimination';
    ...
    testButton.addEventListener('click', testTargetElimination);
    document.body.appendChild(testButton);
    
    // Test function to verify target elimination and history tracking
    function testTargetElimination() {
        console.clear();
        console.log('=== Starting target elimination test ===');
        
        // Make sure we have at least 3 players for testing
        if (gameData.players.length < 3) {
            console.error('Not enough players for test');
            return;
        }
        
        // Get the first 3 players: hunter -> target1 -> target2
        const hunter = gameData.players[0];
        const target1 = gameData.players.find(p => p._id === hunter.target);
        const target2 = target1 ? gameData.players.find(p => p._id === target1.target) : null;
        
        if (!hunter || !target1 || !target2) {
            console.error('Could not find valid hunter/target chain for testing');
            return;
        }
        
        console.log('Initial state:');
        console.log(`${hunter.name} (${hunter._id}) -> ${target1.name} (${target1._id}) -> ${target2.name} (${target2._id})`);
        
        // Store initial state for verification
        const initialHunterTarget = hunter.target;
        const initialTarget1Status = target1.status;
        const initialHunterPreviousTargets = [...(hunter.previousTargets || [])];
        
        // Simulate hunter eliminating target1
        console.log(`\nEliminating ${target1.name} (${target1._id})...`);
        eliminatePlayer(target1._id, hunter._id);
        
        // Check results after a short delay to allow for any async operations
        setTimeout(() => {
            console.log('\n=== Test Results ===');
            
            // 1. Verify target1 is marked as eliminated
            if (target1.status === 'eliminated') {
                console.log('✅ Target marked as eliminated');
            } else {
                console.error('❌ Target not marked as eliminated');
            }
            
            // 2. Verify hunter's target was updated to target2
            if (hunter.target === target2._id) {
                console.log('✅ Hunter received correct new target');
            } else {
                console.error(`❌ Hunter's target not updated correctly. Expected: ${target2._id}, Got: ${hunter.target}`);
            }
            
            // 3. Verify target1 is in hunter's previous targets
            if (hunter.previousTargets?.includes(target1._id)) {
                console.log('✅ Target added to hunter\'s previous targets');
            } else {
                console.error('❌ Target not added to hunter\'s previous targets');
            }
            
            // 4. Verify the UI would update (we can't test UI directly, but we can check the data)
            console.log('ℹ️ Check the UI to ensure the target list updates correctly');
            
            console.log('\n=== Test Complete ===');
            console.log('Note: This was a test. Game state has been modified. Refresh to reset.');
        }, 100);
    }
});
